...
void computeHeight(Node *n) {
  ...
  if (!n->left && !n->right) {
    // std::cout << "Setting height of Leaf node" << std::endl;
    n->height = 0;
    return;
  }
  ...
  if (n->right) { 
    ...
    // std::cout << "Setting height of a Non Leaf node" << std::endl;
    n->height = n->right->height + 1;
  }
  
  if (n->left) {
    computeHeight(n->left);
    int leftSubtreeHeight = n->left->height;
    if (n->height < (leftSubtreeHeight + 1)) {
      // std::cout << "Updating height of a Non Leaf node" << std::endl;
      n->height = leftSubtreeHeight + 1;
    }
  }
